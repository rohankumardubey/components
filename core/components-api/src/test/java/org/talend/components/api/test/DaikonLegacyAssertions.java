...
public class DaikonLegacyAssertions {

    private static final Logger LOGGER = LoggerFactory.getLogger(DaikonLegacyAssertions.class);

    public static Properties checkSerialize(Properties props, ErrorCollector errorCollector) {
        String s = props.toSerialized();
        SerializerDeserializer.Deserialized<Properties> d =
                Properties.Helper.fromSerializedPersistent(s, Properties.class);
     ...
     * @param prop
     *         the property to check for an i18N {@link Property#getDisplayName()}
     * @param parent,
     *         used only for the error message to identify the origin of the property
     */
    static public void checkProperty(final ErrorCollector errorCollector, Property<?> prop, Object parent) {
        ...
        if (checkedProps == null) {
            ...
            checkedProps.accept(new AnyPropertyVisitor() {
                ...
                public void visit(Property prop, Properties parent) {
                    checkProperty(errorCollector, prop, parent);
                }
     ...
     *
     * @param defRegistry
     *         where to get all the definitions
     * @param errorCollector
     *         used to collect all errors at once. @see <a
    ...
    static public void assertAlli18nAreSetup(DefinitionRegistryService defRegistry, ErrorCollector errorCollector) {
        ...
        for (Definition def : allDefs) {
            ...
            errorCollector.checkThat(
                    "missing I18n displayName [" + def.getDisplayName() + "] for definition [" + def.getClass()
                            .getName() + "]",
                    def.getDisplayName(), not(endsWith(Definition.I18N_DISPLAY_NAME_SUFFIX)));
            // check definition title
            errorCollector.checkThat("title for definition [" + def.getClass().getName() + "] must not be null",
                    def.getTitle(),
                    notNullValue());
            errorCollector.checkThat(
     ...
     *
     * @param defRegistry
     *         service to get the components to be checked.
     */
    public static void assertAnIconIsSetup(DefinitionRegistryService defRegistry, ErrorCollector errorCollector) {
        ...
        for (Definition def : allDefs) {
            ...
            if (pngImagePath == null && svgImagePath == null && iconKey == null) {
                errorCollector
                        .addError(new Throwable(
                                "the definition [" + def.getName() + "] must have at least one icon resource."));
            }
            if (pngImagePath != null) {// check that the image resource exists
                ...
                errorCollector
                        .checkThat(
                                "Failed to find the image for path [" + pngImagePath + "] for the definition ["
                                        + def.getName()
            ...
            if (svgImagePath != null) {// check that the image resource exists
                ...
                errorCollector
                        .checkThat(
                                "Failed to find the image for path [" + svgImagePath + "] for the definition ["
                                        + def.getName()
                                        + "].\nIt should be located at ["
    ...
    public static Properties checkAndBeforeActivate(PropertiesService propServ, Form form, String propName,
            Properties props)
            throws Throwable {
    ...
    public static Properties checkAndBeforePresent(PropertiesService propServ, Form form, String propName,
            Properties props)
            throws Throwable {
