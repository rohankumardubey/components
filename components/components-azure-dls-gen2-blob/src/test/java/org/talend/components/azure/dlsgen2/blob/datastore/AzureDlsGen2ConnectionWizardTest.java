...
import org.talend.components.azure.dlsgen2.blob.AzureDlsGen2BlobBase;
...
public class AzureDlsGen2ConnectionWizardTest extends AzureDlsGen2BlobBase {
    ...
    public void testWizard() throws Throwable {
        ...
        for (ComponentWizardDefinition wizardDefinition : wizards) {
            if (wizardDefinition instanceof AzureDlsGen2ConnectionWizardDefinition) {
                wizardDef = wizardDefinition;
                count++;
            }
        }
        assertEquals(1, count);
        assertEquals("Create an Azure DLS Gen2 Connection", wizardDef.getMenuItemName());
        ComponentWizard wiz = getComponentService()
                .getComponentWizard(AzureDlsGen2ConnectionWizardDefinition.COMPONENT_NAME, "nodeAzureDlsGen2");
        assertNotNull(wiz);
        assertEquals("nodeAzureDlsGen2", wiz.getRepositoryLocation());
        ...
        // Main from AzureDlsGen2BlobModuleListProperties
        assertEquals("Azure DLS Gen2 Connection Settings", connFormWizard.getTitle());
        assertEquals("Fill in fields to configure connection.", connFormWizard.getSubtitle());
        AzureDlsGen2BlobDatastoreProperties connProps = (AzureDlsGen2BlobDatastoreProperties) connFormWizard
                .getProperties();
